...
foreach bc_rec ( `cat $1 | tr ',' ':' | tr -d '\r' | tr -d ' ' | sort -u` )
  ...
  set original_sff = ${sample_data_merged_sff}/${db_name}_${col_name}_${bac_id}.sff
  ...
  pushd ${sample_mapping_dir} >& /dev/null
    ...
    foreach key (`ls -1 ${sample_data_merged_sff} | grep "\.[ACGT][ACGT][ACGT][ACGT]\." | cut -d '.' -f 2 | sort -u`)
      set input_read_files = `echo "${input_read_files} -q ${original_sff:r}.${key}.sff"`
    end
    ...
    foreach key (`ls -1 ${sample_data_merged_sff} | grep "\.[ACGT][ACGT][ACGT][ACGT]\." | cut -d '.' -f 2 | sort -u`)
      addRun 454_mapping_best_refs_chimera_check ${original_sff:r}.${key}.sff
    end
